...
function setupIpTc {
   ...
   for n in $5
    do
      ...
      mainHandle=$(echo $targetIp | cut -d. -f3)
      secondHandle=$(echo $targetIp | cut -d. -f4)
      counter=300
      let "k = $mainHandle+$counter"
      Q=$(printf "%02x" $k)
      secondHt=$(printf "%02x" $secondHandle)
...
function setuptc {
  ...
  cmd "tc filter add dev eth0 parent 1: prio 10 handle b1: protocol ip u32 divisor 256"
  ...
  cmd "tc filter add dev lo parent 1: prio 10 handle b1: protocol ip u32 divisor 256"
  ...
  for idIp in {0..255}
    do
      #create other hash table
      let "C = $idIp+$counter"
      Q=$(printf "%x" $C)
      K=$(printf "%x" $idIp)
      cmd "tc filter add dev eth0 parent 1: prio 10 handle ${Q}: protocol ip u32 divisor 256"
      #fills b1
      cmd "tc filter add dev eth0 parent 1: protocol ip prio 5 \
        u32 ht b1:${K}: \
        match ip dst 10.10.${idIp}.0/24 \
        hashkey mask 0x000000ff at 16 \
        link ${Q}:"
    done

   echo "Setting up indirection hashtables for lo"
  ...
  for idIpl in {0..255}
    do
      ...
      Q=$(printf "%x" $C)
      K=$(printf "%x" $idIpl)
      cmd "tc filter add dev lo parent 1: prio 5 handle ${Q}: protocol ip u32 divisor 256"
      #fills b1
      cmd "tc filter add dev lo parent 1: protocol ip prio 5 \
        u32 ht b1:${K}: \
        match ip dst 10.10.${idIpl}.0/24 \
        hashkey mask 0x000000ff at 16 \
        link ${Q}:"
    done  
