...
suspend fun runMobilityAdv(expYaml: YamlNode, proxies: Proxies, dockerConfig: DockerConfig) {
    val expConfig = Yaml.default.decodeFromString<MobilityAdvConfig>(expYaml.contentToString())
    val nExps = expConfig.tcSetup.size * expConfig.nodes.size * expConfig.dataDistribution.size *
            expConfig.readPercents.size * expConfig.migrationPattern.size * expConfig.treeBuilderSubList.size
                ...
                expConfig.dataDistribution.forEach { dataDistribution ->
                    expConfig.readPercents.forEach { readPercent ->
                        expConfig.treeBuilderSubList.forEach sub@{ locationSub ->
                            ...
                            val logsPath =
                                "${expConfig.name}/${nNodes}n_${dataDistribution}_${readPercent}r_${migPattern}_${locationSub}_${tcBaseFileNumber}"

                            if (!File("${dockerConfig.logsFolder}/$logsPath").exists()) {
                                println(
                                    "---------- Running experiment $nExp/$nExps with $tcConfigFile, $nNodes nodes, " +
                                            "$migPattern, $dataDistribution, $readPercent reads $locationSub -------"
                                )
                            } else {
                                println(
                                    "---------- Skipping existing $nExp/$nExps with $tcConfigFile, $nNodes nodes, " +
                                            "$migPattern, $dataDistribution, $readPercent $locationSub reads -------"
                                )
                                return@sub
                            }
                            ...
                            runExp(
                                ...
                                dataDistribution, readPercent, migPattern, locationSub, "/logs/$logsPath"
                            )
...
private suspend fun runExp(
    ...
    locationsMap: Map<Int, Location>, expConfig: MobilityAdvConfig, tcConfigFile: String, nNodes: Int,
    dataDistribution: String, readPercent: Int, migrationPattern: String, locationSub: String, logsPath: String,
) {
    startAllNodes(nodes, locationsMap, logsPath, expConfig.gcThreshold, expConfig.gcInterval, locationSub)
...
private suspend fun startAllClients(
    ...
    logsPath: String, expConfig: MobilityAdvConfig,
) {
    coroutineScope {
...
private suspend fun startAllNodes(
    ...
    locationsMap: Map<Int, Location>,
    logsPath: String, gcThreshold: Int, gcInterval: Int, locationSub: String
) {
