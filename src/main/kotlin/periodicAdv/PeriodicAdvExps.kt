...
suspend fun runPeriodicAdv(expYaml: YamlNode, proxies: Proxies, dockerConfig: DockerConfig) {
    val expConfig = Yaml.default.decodeFromString<PeriodicAdvConfig>(expYaml.contentToString())
    val nExps = expConfig.tcSetup.size * expConfig.nodes.size *
            expConfig.readPercents.size * expConfig.periodicModes.size * expConfig.treeBuilderSubList.size
            ...
            expConfig.periodicModes.forEach { mode ->
                expConfig.readPercents.forEach { readPercent ->
                    expConfig.treeBuilderSubList.forEach sub@{ locationSub ->
                        ...
                        val logsPath =
                            "${expConfig.name}/${nNodes}n_${mode}_${readPercent}r_${locationSub}_${tcBaseFileNumber}"

                        if (!File("${dockerConfig.logsFolder}/$logsPath").exists()) {
                            println(
                                "---------- Running experiment $nExp/$nExps with $tcConfigFile, $nNodes nodes, " +
                                        "$mode, $readPercent reads $locationSub -------"
                            )
                        } else {
                            println(
                        ...
                        runExp(
                            ...
                            readPercent, locationSub, "/logs/$logsPath"
                        )
...
private suspend fun runExp(
    ...
    locationsMap: Map<Int, Location>, expConfig: PeriodicAdvConfig, nNodes: Int, mode: String,
    readPercent: Int, locationSub: String, logsPath: String,
) {
...
private suspend fun startAllNodes(
    ...
    logsPath: String,
    expConfig: PeriodicAdvConfig, locationSub: String
) {
    ...
    coroutineScope {
        ...
        nodes.forEachIndexed { index, container ->
            ...
            val cmd = mutableListOf(
                ...
                "tree_builder_location_sub=$locationSub", "propagate_timeout=500"
            )
