        ...
        expConfig.nodes.forEach { nNodes ->
            ...
            expConfig.dataDistribution.forEach { dataDistribution ->
                expConfig.readPercents.forEach { readPercent ->
                    expConfig.clientPersistence.forEach skip@{ persistence ->
                        nExp++
                        val tcBaseFileNumber = tcConfigFile.split(".")[0].split("_")[1]
                        val logsPath =
                            "${expConfig.name}/${nNodes}n_${dataDistribution}_${readPercent}r_${persistence}p_${tcBaseFileNumber}"
                        ...
                        runExp(
                            ...
                            dataDistribution, readPercent, persistence, "/logs/$logsPath"
                        )
...
private suspend fun runExp(
    ...
    locationsMap: Map<Int, Location>, expConfig: CassandraLatencyConfig, tcConfigFile: String, nNodes: Int,
    dataDistribution: String, readPercent: Int, persistence: Int, logsPath: String,
) {
...
private suspend fun startAllClients(
    ...
    expConfig: CassandraLatencyConfig,
) {
    coroutineScope {
        clients.forEach { container ->
            ...
            val partitions = expConfig.partitions
            val transformedPersistence = when (nNodes){
                1 -> "ONE"
                else -> when(persistence){
                    1 -> "ONE"
                    2 -> "TWO"
                    3 -> "THREE"
                    10000 -> "QUORUM"
                    else -> throw Exception("Invalid persistence $persistence")
                }
            }
            val cmd = mutableListOf(
                ...
                "-p", "status.interval=1",
                "-p", "cassandra.writeconsistencylevel=$transformedPersistence",
